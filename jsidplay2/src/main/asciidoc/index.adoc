...
JSIDPlay2 source code is still downward compatible to the Java 8 API, but unfortunately compatibility is a bit more complicated.
Because of the use of JavaFX as a dependency, that has been compiled with Java 17, we need this at runtime as well for the UI version.

JSIDPlay2 has been tested and is compatible wit Oracle Java 8 (with JavaFX already included), OpenJDK 17 (with JafaFX contained in my release)
and OpenJDK 21 preview release (with JafaFX contained in my release).
The reason, why we are not compatible with Java 11 is, that a compatible JavaFX version is missing and there is no JDK distribution known to me containing JavaFX.

Btw, a separate program (.exe/.sh) is required and available to launch with Oracle Java 8.
...
link:{codebase}jsidplay2-{version}-jsiddevice.jar[Download Network SID Device] (e.g. for link:https://www.acid64.com[ACID64])

link:{codebase}jsidplay2app.apk[Download jsidplay2app] (*deprecated* android app has been replaced by link:https://haendel.ddns.net:8443/static/c64jukebox.vue[C64Jukebox])

== JSIDPlay2 Server
link:https://haendel.ddns.net:8443/static/c64jukebox.vue[JSIDPlay2 - Web Version^]
