...
public class DetectionThread extends Thread {
	...
	public void run() {
		...
		final byte[] responseData = new StringBuilder().append(MAGIC_ID).append(",").append(systemName).append(",")
				.append(osName).append(" ").append(osArch).toString().getBytes();
		...
		while (running) {
			try {
				...
				if (received.startsWith(MAGIC_ID)
						&& (isLocalIp(incomingPacket) || settings.getAllowExternalConnections())) {
