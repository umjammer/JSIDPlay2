...
public class Palette implements IPalette {
	...
	public void calculatePalette(final PaletteEntry[] colors) {
		...
		evenFiltered = calculatePaletteInternal(colors, evenPalette, saturation, phaseShift, tint);
		oddFiltered = calculatePaletteInternal(colors, oddPalette, saturation, -phaseShift, tint);
		...
		for (int above = 0; above < 256; above++) {
			for (int below = 0; below < 256; below++) {
				...
				evenLineTable[idx] = new YUVEntry(belowColor.y, (aboveColor.u + belowColor.u) * 0.5f,
						(aboveColor.v + belowColor.v) * 0.5f).toRGB(brightness, contrast, gamma);

				aboveColor = evenPalette[above];
				belowColor = oddPalette[below];
				oddLineTable[idx] = new YUVEntry(belowColor.y, (aboveColor.u + belowColor.u) * offset * 0.5f,
						(aboveColor.v + belowColor.v) * offset * 0.5f).toRGB(brightness, contrast, gamma);
			}
		}
	}
